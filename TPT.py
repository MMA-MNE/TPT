...
if df.empty:
    ...
    #df_hiv_known= df_hiv_known.groupby(['hiv'],as_index=False).agg({'year': 'sum'})

    #count_hiv_known= df_hiv_known.iloc[0]
    #count_hiv_unknown= df_hiv_known.iloc[1]
    ...
    #colors= {'Known':'yellow','Unk':'red'}
    #fig_hiv_known = px.pie(df_hiv_known,names= 'hiv',values='year',labels={'year':'Patients'},title='HCT testing', color= 'hiv',color_discrete_map= colors)
    #st.plotly_chart(fig_hiv_known)
    ...
    df_tsr['outcome'].replace({'Complete':'Tx success','Incomplete':'Tx not success', 'Develop TB':'Tx not success', 
                          ...
                          inplace=True)
    df_tsr = df_tsr.groupby(['outcome'], as_index =False).agg({'year':'sum'})

    count_tsr_success = df_tsr.iloc[1]
    count_tsr_not_success = df_tsr.iloc[0]
    ...
    colors= {'Tx success':'blue','Tx not success':'grey'}
    fig_tsr = px.pie(df_tsr, names= 'outcome',values='year',labels={'year':'Patients','outcome':'TSR'},
                            title='TSR of TPT cases from previous year',
                    color= 'outcome',color_discrete_map= colors, hole= 0.6)
    #st.plotly_chart(fig_tsr)
